...
curve_threshold = 40
curve_distance_ratio_threshold = 1 / 4
...
class StepProgress:
    def __init__(self, steps, progress):
        self.steps = steps
...
class RunState:
    def __init__(self, params):
        ...
        self.closest_behind_waypoint_index = params['closest_waypoints'][0]
        self.closest_ahead_waypoint_index = params['closest_waypoints'][1]
...
class Sim:
    ...
    def update(self, speed, heading):
        vx = speed * math.cos(math.radians(heading))
        vy = speed * math.sin(math.radians(heading))
        self.ax = (vx - self.vx) / fps
        self.ay = (vy - self.vy) / fps
        if self.heading:
            self.dheading = (heading - self.heading) / fps
...
def center_line_reward(distance_from_center, track_width, curve_upcoming):
    ...
    return math.exp(-bounded_distance_factor * (1-curve_upcoming))
...
def speed_reward(speed, waypoint_slope_val, curve_upcoming, steering_angle):
    speed_ratio = speed / max_speed
    waypoint_speed_factor = waypoint_slope_val * speed
    if curve_upcoming:
        curve_speed_factor = max(curve_upcoming * speed, min_speed)
        speed_reward = ((curve_speed_factor - min_speed) / (max_speed - min_speed) + waypoint_speed_factor) / 2
    else:
        speed_reward = ((speed - min_speed) / (max_speed - min_speed) + waypoint_speed_factor) / 2
    return speed_reward
...
def waypoint_heading_reward(waypoints, closest_waypoints, heading):
    ...
    next_wp = track_waypoints.waypoint_map[closest_waypoints[1]]
    next_next_wp = next_wp.next_waypoint
    x1, y1 = next_wp.x, next_wp.y
    x2, y2 = next_next_wp.x, next_next_wp.y
    xvec = x2 - x1
    yvec = y2 - y1
...
def reward_function(params):
    ...
    heading360 = heading_to_360(heading)
    run_state.update(speed, heading)
    ...
    center_line_val = center_line_reward(distance_from_center, track_width, curve_upcoming)
    ...
    speed_val = speed_reward(speed, waypoint_slope_val, curve_upcoming, steering_angle)
    speed_ratio = speed / max_speed
    max_progress_val = ((steps * max_speed / fps) / track_length)
    progress_val = (progress / 100)
    progress_factor = progress_val / max_progress_val
    state_factor = (center_line_val + waypoint_slope_val + steering_val + speed_val + progress_factor) / 5
    reward = (progress_val + speed_ratio + state_factor) / 3
